...
router.post('/uploadBannerImg', async (ctx, next) => {
	...
	try {
		let options = {
			uri: 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=' + config.appid + '&secret=' +
				config.secret + '',
			json: true
		}
		...
		options = {
			...
			body: {
				"env": "first-6gkha9h9603daedd",
				"query": "db.collection(\"banner\").add({data:{fileId:\"" + file_id + "\",file_time:\"" + new Date() + "\"}})"
			},
			json: true
		}
